 ...
 * License: MIT
 *
 * Generated at 2020-01-23 11:49:30 -0600
 */
(function (angular$1) {
'use strict';
...
function TagsInputDirective($timeout, $document, $window, $q, tagsInputConfig, tiUtil, tiConstants) {
  ...
  function TagList(options, events, onTagAdding, onTagRemoving) {
    ...
    self.reorder = function (prevOffset, newOffset) {
  ...
  return {
    ...
    link: function link(scope, element, attrs, ngModelCtrl) {
      ...
      scope.getTagClass = function (tag, index) {
        ...
        var ret = [scope.tagClass({ $tag: tag, $index: index, $selected: selected }), { selected: selected }];
        ...
        tag: {
          ...
          mousedown: function mousedown(event) {
            if (options.dragtag) {
              ...
              } //go up parents until you find the LI
          ...
          mouseup: function mouseup(event) {
            if (options.dragtag && scope.draggedElement) {
              ...
              for (var i = 0; i < allElems.length; i++) {
                if (allElems[i].style.position == 'relative') prevOffset = i;
              }
      ...
      events.on('tag-added', scope.onTagAdded).on('invalid-tag', scope.onInvalidTag).on('tag-removed', scope.onTagRemoved).on('tag-clicked', scope.onTagClicked).on('tag-dragged', scope.onTagDragged).on('tag-added', function () {
        scope.newTag.text('');
